...
const Navbar = () => {
  ...
  const activeLinkStyle =
    "flex items-center gap-2 bg-orange-500 text-white rounded-xl px-5 py-2 shadow-sm transition-all duration-300 border-l-4 border-r-4 border-b-4 border-l-amber-900 border-r-amber-900 border-b-amber-900";
  ...
  const navLinks = (
    <>
      ...
      <li>
        ...
        >
          <ShoppingCart className="w-4 h-4 block md:hidden" />
          <span className="text-sm font-medium flex gap-1 items-center justify-center">

            <span className="relative text-white text-base font-bold flex gap-1 items-center justify-center ">
              <ShoppingCart /><span className="absolute left-5 -top-3  text-orange-600 text-base font-bold">{cartCount ?? 0}</span>
            </span>
          </span>
        </NavLink>
      </li>
  ...
  return (
    <div className="navbar bg-black/40 backdrop-blur-sm shadow-md fixed top-0 left-0 right-0 z-50 px-4 md:px-20">
      ...
      <div className="navbar-start flex items-center gap-2 space-x-2">
        ...
        >
          <img className="w-10 h-10 hidden md:block" src={logo} alt="logo" />
        ...
        {loading ? (
          ...
          <>
            <button
              ...
              className="px-3 py-1 text-white hover:bg-white/10 bg-orange-500 rounded-xl shadow-sm transition-all duration-300 border-l-4 border-r-4 border-b-4 border-l-amber-900 border-r-amber-900 border-b-amber-900"
            >
              Logout
            </button>
            <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white">
              <img
                src={user?.photoURL || "https://i.ibb.co/G3CNyWmc/image.jpg"}
                alt="User"
          ...
          <div className="absolute left-0 top-0 h-screen w-64 bg-gray-900 text-white shadow-2xl p-4 animate-slideInLeft z-50">
            ...
            <div className="mt-4">
              ...
              {user ? (
                <button
                  onClick={handleLogout}
                  className="w-full px-3 py-2 border border-amber-900 rounded-xl text-white hover:bg-white/10 transition border-l-8 border-r-8 border-b-8 border-l-amber-900 border-r-amber-900 border-b-amber-900"
                >
                  Logout
                </button>
              ) : (
